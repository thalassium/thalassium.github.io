...
// Définition du niveau à afficher 
const DepNode = 5;
...
onMounted(() => {
  ...
  if (node.depth === DepNode) {
    const bounds = calculateSegmentBounds(node, maxDepth, radius);
    node.value = bounds.x1 - bounds.x0; // Utilisation de l'espace total
  }
});
  ...
  const bounds = calculateSegmentBounds(node, maxDepth, radius);
  const siblingsCount = node.parent ? node.parent.children.length -1 : 0;
  console.log(`Node: ${node.data.name}, Siblings Count: ${siblingsCount}`);
  ...
  segmentGroup.append("path")
    ...
    .style("fill-opacity", 1.2);

  drawLabels(segmentGroup, arc, "niv3pa40", emit);
});
</script>
